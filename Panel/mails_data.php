...
if ($con) {
    $sql = "SELECT * FROM email_subscriptions WHERE 1";
    $params = [];
    $types = "";

    if (!empty($datef)) {
        $sql .= " AND subscribed_at >= ?";
            ...
            <div class="ps-2">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb mb-0 p-0" style="list-style: none; display: flex; gap: 8px; align-items: center; font-size: 13px;">
                        ...
                        <li style="font-weight: 600; color: #64748b;">Email Subscriptions</li>
                    </ol>
                </nav>
            </div>
        </div>

        <h4 class="mb-3 text-dark fw-bold">Email Subscriptions</h4>
                        ...
                        <?php if (count($rows) > 0): ?>
                            <?php $count = 0; foreach ($rows as $row): ?>
                                <tr>
                                    ...
                                    <td class="fw-bold text-dark"><?= htmlspecialchars($row['email'] ?? '') ?></td>
                                    <td><?= date('d M Y, h:i A', strtotime($row['subscribed_at'])) ?></td>
                                </tr>
                            <?php endforeach; ?>
                        <?php else: ?>
                            <tr>
                                <td colspan="3" class="text-center py-4 text-muted">No Subscriptions Found</td>
                            </tr>
                        <?php endif; ?>
                        </tbody>
                    </table>
...
function exportToExcel() {
    try {
        ...
        let workbookData = [];
        workbookData.push(["#", "Email ID", "Subscribed At"]);

        const rows = document.querySelectorAll("#datatable-buttons tbody tr");
        rows.forEach((row, index) => {
            const cells = row.cells;
            if (cells.length < 3 || cells[0].innerText.includes("No Subscriptions Found")) return;
        ...
        XLSX.utils.book_append_sheet(wb, ws, "Email_Subscriptions");

        const safeDate = new Date().toISOString().slice(0, 10);
        XLSX.writeFile(wb, "Email_Subscriptions_Report_" + safeDate + ".xlsx");
