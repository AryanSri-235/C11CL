...
if ($con) {
    $sql = "SELECT * FROM callback_requests WHERE 1";
    $params = [];
    $types = "";
    ...
    $sql .= " ORDER BY requested_at DESC LIMIT 1000";
            ...
            <div class="ps-2">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb mb-0 p-0" style="list-style: none; display: flex; gap: 8px; align-items: center; font-size: 13px;">
                        ...
                        <li style="font-weight: 600; color: #64748b;">Callback Requests</li>
                    </ol>
                </nav>
            </div>
        </div>

        <h4 class="mb-3 text-dark fw-bold">Callback Requests</h4>
                        ...
                        <?php if (count($rows) > 0): ?>
                            <?php $count = 0; foreach ($rows as $row): ?>
                                <tr>
                                    ...
                                    <td class="fw-bold text-primary" style="font-family: monospace; font-size: 14px;"><?= htmlspecialchars($row['phone'] ?? '') ?></td>
...
function exportToExcel() {
    try {
        ...
        let workbookData = [];
        workbookData.push(["#", "Name", "Phone", "Requested At"]);

        const rows = document.querySelectorAll("#datatable-buttons tbody tr");
        rows.forEach((row, index) => {
            const cells = row.cells;
            if (cells.length < 4 || cells[0].innerText.includes("No Requests Found")) return;
        ...
        XLSX.utils.book_append_sheet(wb, ws, "Callback_Requests");

        const safeDate = new Date().toISOString().slice(0, 10);
        XLSX.writeFile(wb, "Callback_Requests_Report_" + safeDate + ".xlsx");
