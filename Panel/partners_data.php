...
if ($con) {
    $sql = "SELECT * FROM academy_partners WHERE 1";
    $params = [];
    $types = "";
            ...
            <div class="ps-2">
                <nav aria-label="breadcrumb">
                    <ol class="breadcrumb mb-0 p-0" style="list-style: none; display: flex; gap: 8px; align-items: center; font-size: 13px;">
                        ...
                        <li style="font-weight: 600; color: #64748b;">Academy Partners</li>
                    </ol>
                </nav>
            </div>
        </div>

        <h4 class="mb-3 text-dark fw-bold">Academy Partners Inquiries</h4>
            ...
            <div class="card-body">
                <div class="table-responsive">
                    <table id="datatable-buttons" class="table table-hover table-striped">
                        ...
                        <?php if (count($rows) > 0): ?>
                            <?php $count = 0; foreach ($rows as $row): ?>
                                <tr>
                                    ...
                                    <td class="fw-bold text-dark"><?= htmlspecialchars($row['academy_name'] ?? '') ?></td>
                                    <td><?= htmlspecialchars($row['owner_name'] ?? '') ?></td>
                                    <td><?= htmlspecialchars($row['phone'] ?? '') ?></td>
                                    <td><?= htmlspecialchars($row['location'] ?? '') ?></td>
                                    <td><?= htmlspecialchars($row['coach_name'] ?? '') ?></td>
                                    <td><?= htmlspecialchars($row['active_players'] ?? '') ?></td>
                        ...
                        <?php else: ?>
                            <tr>
                                <td colspan="10" class="text-center py-4 text-muted">No Academy Partners Found</td>
                            </tr>
                        <?php endif; ?>
                        </tbody>
                    </table>
...
function exportToExcel() {
    try {
        ...
        let workbookData = [];
        workbookData.push(["#", "Academy Name", "Owner Name", "Phone", "Location", "Head Coach", "Active Players", "Years", "Reg No.", "Submitted At"]);

        const rows = document.querySelectorAll("#datatable-buttons tbody tr");
        rows.forEach((row, index) => {
            const cells = row.cells;
            if (cells.length < 10 || cells[0].innerText.includes("No Academy Partners Found")) return;
        ...
        XLSX.utils.book_append_sheet(wb, ws, "Academy_Partners");

        const safeDate = new Date().toISOString().slice(0, 10);
        XLSX.writeFile(wb, "Academy_Partners_Report_" + safeDate + ".xlsx");
