...
$sql = "UPDATE register SET age = '$age', player = '$player', up = '$date', state = '$state', city = '$city', ref = '$ref' WHERE reg = '$reg'";
        $con->query($sql);
        $_SESSION['payreg'] = $reg;
        //echo $reg . $age . $player;
...
(name, reg, age, mobile, email, player, state, city, ref, paydate, paytime, created_at, mailsent, up, wasent, regCount, status) 
VALUES 
('$name', '$reg', '$age', '$mobile', '$email', '$player', '$state', '$city', '$ref', 
 ...
 if ($con->query($sql) === TRUE) {
        ...
        $_SESSION['payreg'] = $reg;
        header('Location: ../payment_discount/pay.php');
        exit();
    } else {
