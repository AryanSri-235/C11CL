...
<link rel="canonical" href="https://c11cl.com/payments/" />
...
<meta property="og:url" content="https://c11cl.com/payments/" />
...
<meta property="twitter:url" content="https://c11cl.com/payments/" />
    ...
    <div style="background:#fff; width:95%; max-width:550px; margin:30px auto; padding:30px; border-radius:15px; position:relative; font-family: 'Segoe UI', sans-serif; box-shadow: 0 10px 30px rgba(0,0,0,0.3);">
        ...
        <form id="updateForm" action="../update_details.php" method="POST">
            ...
            <div style="margin-top:20px;">
                <button type="submit" id="saveBtn" style="width:100%; background:#28a745; color:#fff; border:none; padding:15px; cursor:pointer; font-weight:800; border-radius:8px; display:none; text-transform:uppercase;">💾 Save & Update Profile</button>
            </div>
        </form>

        <form id="payForm" method="POST" action="../second_submit_test.php" style="margin-top:12px;">
    <input type="hidden" name="reg" id="payRegId">
    
    <div id="offerLabel" style="text-align:center; margin-bottom:8px;">
        <span style="color:#666; font-weight:800; font-size:12px; letter-spacing:1px; text-transform:uppercase;">
            Verified Profile - Phase 2 Enrollment
        </span>
    </div>

    <button type="submit" id="payBtn" style="width:100%; background:#e31e24; color:#fff; border:none; padding:18px; cursor:pointer; font-weight:800; border-radius:8px; font-size:1.1rem; text-transform:uppercase; letter-spacing:1px; box-shadow: 0 4px 12px rgba(227, 30, 36, 0.2);">
        Continue to Pay Phase 2 Fees 
        <span style="font-size: 1.2rem; margin-left:10px; border-left: 1px solid rgba(255,255,255,0.3); padding-left: 10px;">
            ₹8999/-
        </span> ➔
    </button>
</form>
...
function showPlayerProfile(data) {
    ...
    document.getElementById('payRegId').value = data.reg; // Pay form ke liye ID
...
function enableEditing() {
    // 1. Pehle purani values ko variables mein save kar lein
    const locValue = document.querySelector('#locBox input').value; // Example: "Bihar - Patna"
    const [savedState, savedCity] = locValue.split(' - ').map(s => s.trim());
    const savedRole = document.getElementsByName('role')[0].value; // Example: "Bowler"

    // 2. Inputs ko editable banayein
    const inputs = document.querySelectorAll('.editable-field');
    inputs.forEach(i => { i.readOnly = false; i.classList.add('editing-active'); });

    // 3. Location Box mein dropdowns dalein
    ...
    // 4. Speciality (Role) mein dropdown dalein
    document.getElementById('roleBox').innerHTML = `
        <label style="font-size:11px; font-weight:800; color:#e31e24;">SPECIALITY</label>
        <select id="roleSelect" name="role" style="width:100%; padding:12px; border:1px solid #ddd; margin-top:5px;" required>
